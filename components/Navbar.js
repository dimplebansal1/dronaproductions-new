            ...
            {NAV.map((item) => {
              ...
              return (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className={`group relative block whitespace-nowrap rounded-full px-3.5 py-2 text-[0.72rem] font-medium uppercase tracking-[0.11em] transition-colors duration-300 ${active
                      ? "text-gold-soft"
                      : "text-mist hover:text-fog"
                      }`}
                  >
                    <span
                      className={`absolute inset-0 rounded-full bg-fog/[0.06] transition-opacity duration-300 ${active ? "opacity-100" : "opacity-0 group-hover:opacity-100"
                        }`}
                    />
