        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-16 md:mb-24">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <span className="eyebrow">Case Studies & Proof</span>
            </div>
          ...
          <div className="flex flex-wrap gap-2 border-b border-zinc-900 pb-2">
            {categories.map((category) => (
              <button
                ...
                className={`text-xs md:text-sm font-semibold tracking-widest uppercase px-4 py-2.5 transition-all duration-300 border-b-2 ${
                  selectedCategory === category
                    ? "text-gold-soft"
                    : "text-zinc-500 hover:text-zinc-300"
                }`}
                style={{ borderBottomColor: selectedCategory === category ? "var(--color-gold-soft)" : "transparent" }}
              >
                {category}
              </button>
            ))}
            ...
            >
              ...
              <div className="relative z-10 w-full transition-transform duration-300 group-hover:-translate-y-2">
                ...
                <div className="border-t border-zinc-900 pt-4">
                  <div>
                    <span className="text-xs md:text-sm font-bold tracking-widest text-zinc-400 uppercase block mb-2">
                      Services Provided
                    </span>
                    <div className="flex flex-wrap gap-1.5">
                      {project.services.map((service, idx) => (
                        <span
                          key={idx}
                          className="text-xs font-normal text-zinc-300 bg-zinc-900/60 px-2.5 py-1 border border-zinc-800/60"
                        >
                          {service}
                        </span>
                      ))}
