...
// right-hand bento (3-col grid → 4 clean rows).
const IMAGE_TILES = {
  3: { src: IMAGES.lighting, label: "Intelligent Lighting", icon: "Lightbulb" },
...
function PhotoTile({ tile, delay, className = "col-span-1 sm:col-span-2" }) {
  return (
    <Reveal delay={delay} className={className}>
      <Link
        href="/production"
        className="group relative flex h-full min-h-[14.5rem] overflow-hidden rounded-sm border border-line"
      >
        <Image
          src={tile.src}
          alt={tile.label}
...
function TechCard({ item, index, delay }) {
  ...
  return (
    <Reveal delay={delay} className="col-span-1">
      <div className="card group relative flex h-full min-h-[14.5rem] flex-col justify-between overflow-hidden rounded-sm p-6 transition-all duration-500 hover:border-gold/30 hover:shadow-[0_20px_50px_-20px_rgba(212,175,55,0.12)]">
...
export default function ProductionShowcase({ services }) {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-12">
      {/* Feature panel */}
      <div className="lg:col-span-4 lg:sticky lg:top-28 lg:self-center">
        <Reveal>
          <div className="group relative flex min-h-[26rem] overflow-hidden rounded-sm border border-line lg:min-h-[34rem]">
          <Image
